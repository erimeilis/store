...
// .wrangler/tmp/bundle-G11fZe/checked-fetch.js
  ...
  return async (c, next) => {
    ...
    try {
      ...
      if (!tokenResult) {
        ...
        let userContext2 = { id: "anonymous", permissions: [] };
        if (token === fullAccessToken) {
          hasPermission2 = true;
          userContext2 = { id: "full-access-user", permissions: ["read", "write"] };
        } else if (token === readOnlyToken) {
          hasPermission2 = requiredPermission === "read";
          userContext2 = { id: "read-only-user", permissions: ["read"] };
        }
        if (!hasPermission2) {
          return c.json({
            error: "Unauthorized",
...
app.get("/api/access/users", writeAuthMiddleware, async (c) => {
  try {
    const { results } = await c.env.DB.prepare(
      "SELECT id, email, added_by, added_at, is_admin, notes, created_at, updated_at FROM allowed_users ORDER BY created_at DESC"
    ).all();
    return c.json({
      users: results,
      count: results.length
...
app.get("/api/access/domains", writeAuthMiddleware, async (c) => {
  try {
    const { results } = await c.env.DB.prepare(
      "SELECT id, domain, added_by, added_at, notes, created_at, updated_at FROM allowed_domains ORDER BY created_at DESC"
    ).all();
    return c.json({
      domains: results,
      count: results.length
...
app.post("/api/access/users", writeAuthMiddleware, async (c) => {
  try {
    const body = await c.req.json();
    ...
    const addedBy = user?.id || "unknown";
    const result = await c.env.DB.prepare(
      "INSERT INTO allowed_users (email, added_by, is_admin, notes) VALUES (?, ?, ?, ?)"
    ).bind(
      body.email.toLowerCase().trim(),
      addedBy,
      Boolean(body.is_admin) || false,
      body.notes || null
    ).run();
    const createdUser = await c.env.DB.prepare(
      "SELECT id, email, added_by, added_at, is_admin, notes, created_at, updated_at FROM allowed_users WHERE id = ?"
    ).bind(result.meta.last_row_id).first();
    return c.json({
      user: createdUser,
    ...
    if (error instanceof Error && error.message.includes("UNIQUE constraint failed")) {
      return c.json({
        ...
        message: "This email address is already in the allowed users list"
      }, 409);
    }
    if (error instanceof SyntaxError) {
      return c.json({
...
app.post("/api/access/domains", writeAuthMiddleware, async (c) => {
  try {
    const body = await c.req.json();
    ...
    const cleanDomain = body.domain.startsWith("@") ? body.domain.slice(1) : body.domain;
    ...
    const addedBy = user?.id || "unknown";
    const result = await c.env.DB.prepare(
      "INSERT INTO allowed_domains (domain, added_by, notes) VALUES (?, ?, ?)"
    ).bind(
      cleanDomain.toLowerCase().trim(),
      addedBy,
      body.notes || null
    ).run();
    const createdDomain = await c.env.DB.prepare(
      "SELECT id, domain, added_by, added_at, notes, created_at, updated_at FROM allowed_domains WHERE id = ?"
    ).bind(result.meta.last_row_id).first();
    return c.json({
    ...
    if (error instanceof Error && error.message.includes("UNIQUE constraint failed")) {
      return c.json({
        ...
        message: "This domain is already in the allowed domains list"
      }, 409);
    }
    if (error instanceof SyntaxError) {
      return c.json({
...
app.delete("/api/access/users/:id", writeAuthMiddleware, async (c) => {
  try {
    const id = parseInt(c.req.param("id"));
    if (isNaN(id)) {
      return c.json({
...
app.delete("/api/access/domains/:id", writeAuthMiddleware, async (c) => {
  try {
    const id = parseInt(c.req.param("id"));
    if (isNaN(id)) {
      return c.json({
    ...
    const existingDomain = await c.env.DB.prepare(
      "SELECT * FROM allowed_domains WHERE id = ?"
    ).bind(id).first();
    if (!existingDomain) {
      return c.json({
        error: "Domain not found",
        message: `Domain with ID ${id} does not exist`
      }, 404);
    }
    await c.env.DB.prepare(
...
app.get("/api/access/status", readAuthMiddleware, async (c) => {
  try {
    const user = c.get("user");
    if (!user?.id) {
      return c.json({
...
// .wrangler/tmp/bundle-G11fZe/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-G11fZe/middleware-loader.entry.ts
