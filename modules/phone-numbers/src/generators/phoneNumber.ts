import type { ModuleDataGenerator } from '../../types.js'

/**
 * Country-specific phone number generation patterns
 */
interface CountryPattern {
  code: string
  areaCodeRanges: Array<{ min: number; max: number }>
  subscriberLength: number
  format: (areaCode: string, subscriber: string) => string
}

const COUNTRY_PATTERNS: Record<string, CountryPattern> = {
  US: {
    code: '+1',
    areaCodeRanges: [
  ...
  FR: {
    ...
    subscriberLength: 8,
    format: (area, sub) =>
      `+33 ${area} ${sub.slice(0, 2)} ${sub.slice(2, 4)} ${sub.slice(4, 6)} ${sub.slice(6)}`,
  },
  AU: {
    code: '+61',
    areaCodeRanges: [
 ...
 */
const US_CARRIER_PREFIXES: Record<string, string[]> = {
 ...
 */
export const phoneNumberGenerator: ModuleDataGenerator = {
  id: 'phone-number',
  ...
  generate(count: number, options?: Record<string, unknown>): string[] {
    const country = (options?.country as string) || 'US'
    const format = (options?.format as string) || 'international'
    ...
    for (let i = 0; i < count; i++) {
      // Generate area code
      const areaCodeRange = pattern.areaCodeRanges[
        randomInRange(0, pattern.areaCodeRanges.length - 1)
      ]
  ...
  preview(options?: Record<string, unknown>): string {
    const samples = this.generate(3, options) as string[]
    return samples.join(', ')
  },
}
 ...
 */
export const didNumberGenerator: ModuleDataGenerator = {
  id: 'did-number',
  ...
  generate(count: number, options?: Record<string, unknown>): string[] {
    const country = (options?.country as string) || 'US'
    const carrier = (options?.carrier as string) || 'any'
    const includeExtension = options?.includeExtension as boolean
    const sequential = options?.sequential as boolean
