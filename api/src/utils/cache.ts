 ...
 * Returns a hex string of 8 characters max
 */
...
export class CacheService {
  ...
  static columnValuesKey(
    ...
    // Hash the filters (can have long values)
    const filterStr = Object.entries(filters)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([k, v]) => `${k}=${v}`)
      .join('&');
    const filterHash = filterStr ? hashString(filterStr) : '0';

    // Final key: "values:{columnName}:{tableHash}:{filterHash}"
    // Max length: ~50 chars, well under 512 byte limit
    return `values:${columnName}:${tableIdsHash}:${filterHash}`;
  }
}
