...
import { validateDataset, getValidationRules, registerModuleColumnTypes } from '@/services/validationService.js'
...
app.get('/:id/validate', adminReadAuthMiddleware, async (c) => {
  try {
    ...
    // Initialize modules and register column types for validation
    const moduleManager = createModuleManager(c.env)
    await moduleManager.initialize()
    const moduleColumnTypes = moduleManager.getColumnTypes()
    registerModuleColumnTypes(moduleColumnTypes)
