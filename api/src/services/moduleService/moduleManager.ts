...
export class ModuleManagerService implements ModuleManager {
  ...
  async activate(moduleId: string): Promise<void> {
    ...
    // Validate required settings before activation
    const validation = this.validateRequiredSettings(module)
    if (!validation.valid) {
      throw new Error(
        `Cannot activate module: missing required settings: ${validation.missingSettings.join(', ')}`
      )
    }
   ...
   */
  private validateRequiredSettings(module: InstalledModule): {
    valid: boolean
    missingSettings: string[]
  } {
    const missingSettings: string[] = []

    if (!module.manifest.settings) {
      return { valid: true, missingSettings: [] }
    }

    for (const setting of module.manifest.settings) {
      if (setting.required) {
        const value = module.settings[setting.id]
        // Check if the value is empty/undefined/null
        if (value === undefined || value === null || value === '') {
          missingSettings.push(setting.displayName || setting.id)
        }
      }
    }
  ...
  /**
   * Get the configuration status of a module
   * Returns whether all required settings are configured
   */
  async getConfigurationStatus(moduleId: string): Promise<{
    configured: boolean
    missingSettings: string[]
    allSettings: Array<{
      id: string
    ...
    if (module.manifest.settings) {
      for (const setting of module.manifest.settings) {
        ...
        allSettings.push({
          id: setting.id,
          displayName: setting.displayName || setting.id,
